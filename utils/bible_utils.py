...
def getChaptersForBook(bookId):
    chapters = BOOK_CHAPTER_VERSES[bookId]
    outputList = []
    for chapter, v in chapters.items():
        outputList.append(chapter)
    return outputList

def getBookList(newTestament):
    key = 'oldTestament'
    if newTestament:
        key = 'newTestament'
    books = BIBLE_BOOKS[key]
    outputList = []
